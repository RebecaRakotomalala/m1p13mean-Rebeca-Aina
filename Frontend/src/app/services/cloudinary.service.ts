        ...
        .then(base64 => {
          ...
          this.http.post<UploadResponse>(`${this.apiUrl}/upload/image`, payload)
            .subscribe({
              ...
              error: (error) => {
                console.error('Erreur upload HTTP:', error);
                // Passer l'erreur complète pour un meilleur debugging
                const httpError = {
                  ...error,
                  status: error.status,
                  statusText: error.statusText,
                  error: error.error,
                  message: error.message || error.error?.error || error.error?.message || 'Erreur inconnue'
                };
