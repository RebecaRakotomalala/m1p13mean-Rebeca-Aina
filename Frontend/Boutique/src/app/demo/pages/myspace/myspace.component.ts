      ...
      next: (imageUrl) => {
        ...
        this.boutiqueService.updateBoutique(this.boutique!._id!, {
          ...
          error: (error) => {
            ...
            alert('Erreur lors de la mise à jour du logo: ' + (error.error?.message || error.message));
            setTimeout(() => {
              this.uploadingLogo = false;
              this.uploadProgress = 0;
              this.cdr.detectChanges();
  ...
  onGallerySelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (oversizedFiles.length > 0) {
        alert('Certains fichiers sont trop volumineux (max 5MB par fichier)');
        return;
      }
  ...
  uploadGallery(files: File[]): void {
    ...
    this.cloudinaryService.uploadMultipleImages(files, 'boutiques/galerie').subscribe({
      next: (imageUrls) => {
        ...
        this.boutiqueService.updateBoutique(this.boutique!._id!, {
          ...
          next: (updateResponse) => {
            if (updateResponse.success && updateResponse.boutique) {
              this.boutique = updateResponse.boutique;
              alert(`${imageUrls.length} photo(s) ajoutée(s) avec succès!`);
            }
            setTimeout(() => {
          ...
          error: (error) => {
            ...
            alert('Erreur lors de la mise à jour de la galerie: ' + (error.error?.message || error.message));
            setTimeout(() => {
              this.uploadingGallery = false;
              this.uploadProgress = 0;
              this.cdr.detectChanges();
      ...
      error: (error) => {
        ...
        const errorMessage = error.error?.error || error.error?.message || error.message || 'Erreur inconnue';
        alert('Erreur lors de l\'upload des images: ' + errorMessage);
        setTimeout(() => {
          this.uploadingGallery = false;
          this.uploadProgress = 0;
          this.cdr.detectChanges();
